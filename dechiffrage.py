...
def freq_bigrammes(chaine, alphabet):
    """
    Renvoie le tableau 2D des fréquences d'apparition des lettres et des
    bigrammes (suite de deux lettres consécutives).
    """
    n = len(alphabet)
    m = len(chaine)
...
def affiche_freq_lettres(freq, A):
    """
    Affiche l'histogramme des fréquences des lettres en français à partir
...
def calcul_P(chaine, alphabet):
    """
    Renvoie la matrice des proba Pxy (proba d'avoir l'enchaînement des deux
    caractères x et y).
    """
    n = len(alphabet)
...
def L(chaine, P):
    """
    Renvoie la vraisemblance d'une chaîne.
    Pour chaque caractère car du texte, on trouve son indice dans l'alphabet,
    ainsi que l'indice du caractère suivant.
    On en déduit la proba P correspondant à cet enchaînement.
    P est la matrice des probas calculée sur un texte de référence.
    """
    S = 0
...
def remplacement(chaine, alphabet):
    """
    Réalise une permutation circulaire sur la chaine pour trois lettres de
    l'alphabet tirées au hasard.
    """
...
def dechiffrement(texte, n, P, alphabet):
    """
    Déchiffre le message par une méthode de recuit simulé.
    n est le nombre d'itérations de l'algorithme.
    P est la matrice des fréquences des bigrammes.
    alphabet est une liste des caractères autorisés.
    """
    liste_L = []
    i_opt = 0
    for i in range(1, n):
